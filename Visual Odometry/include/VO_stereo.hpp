...
public:
    ...
    VisualOdometry_stereo(std::string new_data_directory);
    // ~VisualOdometry_monocular();

    // int motion_estimation(std::vector<cv::Point2f>& points2D_current,
    ...
    int triangulate(cv::Mat& projection_matrix_current,
                    cv::Mat& projection_matrix_previous,
                    std::vector<cv::Point2f>& points2D_current,
                    std::vector<cv::Point2f>& points2D_previous,
                    std::vector<cv::Point3f>& points3D);

    cv::Mat find_Rti(std::vector<cv::Point2f>& points2D_current,
                 std::vector<cv::Point3f>& points3D,
                 cv::Mat& Rti);
